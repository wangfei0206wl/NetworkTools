...
/*! ping packet.
 *  \details fetch the ping packet.
 *
 *      the default ping packet has ICMP header, include a standard 56 byte payload (resulting
 *      in a standard 64 byte ping).
 *  \param data Some data to include in the ping packet, after the ICMP header, or nil if you
 *      want the packet to include a standard 56 byte payload (resulting in a standard 64 byte
 *      ping).
 */
 ...
 *  \param sequenceNumber The ICMP sequence number of that packet.
 *  \param timeToLive route jumps.
 *  \param timeElapsed time elapsed for this packet response.
 */
...
enum {
    ...
    ICMPv4TypeEchoTimeout = 11,         ///< The ICMP `type` for a ping response; in this case `code` is always 0.
};
...
enum {
    ...
    ICMPv6TypeEchoTimeout = 3,          ///< The ICMP `type` for a ping response; in this case `code` is always 0.
};
...
extern uint16_t in_cksum(const void *buffer, size_t bufferLen);
