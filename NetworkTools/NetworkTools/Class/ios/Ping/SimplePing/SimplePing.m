...
- (void)sendPingWithData:(NSData *)data {
    ...
    packet = [self packetWithPingData:data];
...
- (NSData *)packetWithPingData:(nullable NSData *)data {
    // 将ping的packet构造单独提出
    NSData *                payload;
    NSData *                packet;
...
+ (NSUInteger)icmpHeaderOffsetInIPv4Packet:(NSData *)packet timeToLive:(uint16_t *)timeToLive {
...
- (BOOL)validatePing4ResponsePacket:(NSMutableData *)packet sequenceNumber:(uint16_t *)sequenceNumberPtr timeToLive:(uint16_t *)timeToLive {
    BOOL                result;
...
- (BOOL)validatePingResponsePacket:(NSMutableData *)packet sequenceNumber:(uint16_t *)sequenceNumberPtr timeToLive:(uint16_t *)timeToLive {
    BOOL        result;
...
- (void)readData {
    ...
    if (bytesRead > 0) {
        ...
        if ( [self validatePingResponsePacket:packet sequenceNumber:&sequenceNumber timeToLive:&timeToLive] ) {
            if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(simplePing:didReceivePingResponsePacket:sequenceNumber:timeToLive:timeElapsed:)] ) {
                NSTimeInterval timeElapsed = [[NSDate date] timeIntervalSinceDate:self.pingStartDate];
                [strongDelegate simplePing:self didReceivePingResponsePacket:packet sequenceNumber:sequenceNumber timeToLive:timeToLive timeElapsed:timeElapsed];
            }
        } else {
